...
компиляция файла ассемблер с помощью `gcc` производится опцией `-S`, полученный файл будет иметь расширение `.s`. Примечательно, что такой код можно открыть и посмотреть в блокноте или другом текстовом редакторе.

скомпилировать *переносимый исходный файл* `.o` или `.obj` в `windows` можно с помощью опции `-o`. Но делает это утилита `as`! Для той же операции в `gcc` используется опция `-c` иными словами . Скомпилированный файл обычным блокнотом уже не открыть, нужна какая-нибудь программа *дизассемблер*( которая сгенерирует подобие ассемблерного кода но уже для исполняемого файла) - например 
```bash
objdump
readelf
```
...
как видно, можно посмотреть таблицу символов объектного файла тремя способами, через утилиту `nm` или `objdump -t (--syms)` или `readelf -s`
```c
