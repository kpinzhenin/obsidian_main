...
hd0, 0 # Hard disk number 0 and partition number 0
```
...
/etc/grub.d/  содержит набора скриптов которые выполняются при запуске командой `grub2-mkconfig`. Создается впечатление, что содержащиеся файлы добавляют контекста в 

grub.cfg к слову содержит меню выбора загрузчика операционной системы

думаю основная задача `GRUB` загрузить ядро ОС и инициализитовать память.